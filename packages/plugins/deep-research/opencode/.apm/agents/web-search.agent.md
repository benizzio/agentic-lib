---
# Generated from source/agents/web-search.md; do not edit.
...
1. **Query Generation Phase**: When given a topic or problem, you will:
   ...
   **Scenario-Specific Query Strategies (MANDATORY Module Loading)**:
   Before searching the web or fetching a page, locate and read the relevant strategy module(s) under the installed `research` skill's `references/web-search-modules/` directory. Resolve this resource location relative to the installed `research/SKILL.md`, rather than assuming a harness-specific absolute path. Based on the research type, read the corresponding file(s):
   ...
   DO NOT skip this step. DO NOT search the web or fetch a page before loading at least one module.
